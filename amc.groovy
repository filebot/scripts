	...
	def getReportMessage = { 
		'''<!DOCTYPE html>\n''' + XML {
			html {
				...
				body {
					p {
						span("FileBot finished processing"); b(getReportSubject()); span("(${renameLog.size()} files).")
					}
					hr()
					section {
						table {
					...
					hr()
					small("// Generated by ${Settings.applicationIdentifier} on ${InetAddress.localHost.hostName} at ${now}")
				}
			}
		}
	}
