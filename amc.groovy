...
def getReportSubject = { tryQuietly { ut_title } ?: input.collect{ relativeInputPath(it) as File }*.getRoot()*.getNameWithoutExtension().unique().sort{ it.toLowerCase() }.join(', ').take(100) }
def getReportMessage = { 
	...
	XML {
		html {
			...
			body {
				p("FileBot finished processing ${getReportSubject()} (${renameLog.size()} files).")
				hr(); table {
				...
				hr(); table {
					...
					renameLog.each{ from, to ->
						tr { [from.name, to.name].each{ cell -> td{ nobr{ code(cell) } } } }
					}
				}
