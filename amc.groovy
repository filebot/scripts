...
if (pushover || pushbullet ) { include('lib/ws') }
...
// messages used for email / bulletpoint reports
		...
		html {
			head {
				style('''
					...
					table{font-family:Arial,Helvetica,sans-serif;color:#666;font-size:12px;text-shadow:1px 1px 0 #fff;background:#eaebec;margin:15px;border:#ccc 1px solid;-moz-border-radius:3px;-webkit-border-radius:3px;border-radius:3px;-moz-box-shadow:0 1px 2px #d1d1d1;-webkit-box-shadow:0 1px 2px #d1d1d1;box-shadow:0 1px 2px #d1d1d1}
					table th{padding:15px;border-top:1px solid #fafafa;border-bottom:1px solid #e0e0e0;background:#ededed;background:-webkit-gradient(linear,left top,left bottom,from(#ededed),to(#ebebeb));background:-moz-linear-gradient(top,#ededed,#ebebeb)}
					...
					table td{padding:15px;border-top:1px solid #fff;border-bottom:1px solid #e0e0e0;border-left:1px solid #e0e0e0;background:#fafafa;background:-webkit-gradient(linear,left top,left bottom,from(#fbfbfb),to(#fafafa));background:-moz-linear-gradient(top,#fbfbfb,#fafafa);white-space:nowrap}
				...
				''') // @see http://johnsardine.com/freebies/dl-html-css/simple-little-tab/
			}
			body {
				p {
				...
				hr(); table {
					th('Original Name'); th('New Name')
					renameLog.each{ from, to ->
						tr { [from.name, to.name].each{ cell -> td(cell) } }
					}
				}
				hr(); small("// Generated by ${net.sourceforge.filebot.Settings.applicationIdentifier} on ${InetAddress.localHost.hostName} at ${new Date().dateTimeString}")
			}
		}
	}
}
...
if (pushbullet) {
	...
	PushBullet(pushbullet).sendHtml("[FileBot] ${getReportSubject()}", getReportMessage())
}

// send email report
if (gmail) {
	sendGmail(
		subject: "[FileBot] ${getReportSubject()}",
