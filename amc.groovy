...
def forceSeries = { f ->
	label =~ /^(?i:TV|Show|Series|Documetary)/ || f.dir.listPath().any{ it.name ==~ /(?i:TV.Shows)/ } || f.path =~ /(?i:tvs-|tvp-|E{2,3}|EP\d{1,3}|Season\D?\d{1,2}|\d{4}.S\d{2})/ || parseEpisodeNumber(f.path, true) || parseDate(f.path)
}
