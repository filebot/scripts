...
class DiscordClient {
	...
	def postRenameMap = { subject, map ->
		...
		def json = [
			content: "FileBot finished processing **${subject}** (${map.size()} files).",
			embeds: map.collect{ from, to -> [to.parentFile, from.name, to.name] }.groupBy{ it[0] }.collect{ group, names ->
				[
			...
			} + ["footer": [text: "Generated by ${Settings.applicationIdentifier} on ${InetAddress.localHost.hostName} at ${new Date()}", icon_url: 'https://app.filebot.net/avatar.png']]
		]
